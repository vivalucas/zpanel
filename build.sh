...
_build() {
  ...
  if [ -n "$VERSION" ]; then
    outPath="zpanel_${VERSION}_${os}_${arch}"
  elif [ -n "$LATEST_TAG" ]; then
    outPath="zpanel_${LATEST_TAG}_${os}_${arch}"
  else
    outPath="zpanel_${COMMIT_SHA}_${os}_${arch}"
  fi
  outname="${pathRelease}/${outPath}/zpanel"
  go build -o "${outname}" --ldflags="-X zpanel/global.RUNCODE=release" main.go
  ...
  if [ "$os" = "windows" ]; then
    mv $outname $outPath/zpanel.exe
    zip -r "${pathRelease}/${outPath}.zip" $outPath
  else
    mv $outname $outPath/zpanel
...
buildReleaseLinuxMusl() {
  cd $REPO/service
  ldflags="-X zpanel/global.RUNCODE=release"
  pathRelease=$REPO/release
  ...
  for i in "${!OS_ARCHES[@]}"; do
    ...
    if [ -n "$VERSION" ]; then
      outPath="zpanel_${VERSION}_${GOOS}_musl_${GOARCH}"
    elif [ -n "$LATEST_TAG" ]; then
      outPath="zpanel_${LATEST_TAG}_${GOOS}_musl_${GOARCH}"
    else
      outPath="zpanel_${COMMIT_SHA}_${GOOS}_musl_${GOARCH}"
    fi
