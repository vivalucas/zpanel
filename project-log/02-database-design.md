...
- 新增 `import_receipt`：`user_id` + `request_id` 联合主键，`payload_hash`（SHA-256）。与导入数据在同一事务内提交；不储存原始配置或凭据，用于网络重试幂等。收据随完整数据库备份保留。
- `file_reference` 尚未作为唯一真实来源。删除/详情核查 `item_icon`、`user_config`、`user`、`module_config`、`system_setting` 的现有 URL/文件 ID 字段及引用表，兼容已存 JSON 数据，不进行破坏性迁移。
- 替换资源仅更新调用者拥有的引用（管理员额外可更新站点配置），原文件保留；删除前无引用才允许物理清理。
- 相关 API 写入在单进程内互斥，防止引用核查与保存交错；拒绝重新保存已软删除资源的本地 URL。多实例共享写入需未来引入跨实例协调。
- 密码哈希更新与 session 撤销必须共同成功；失败回滚密码。角色修改和删除事务至少保留一个启用管理员，MySQL 使用管理员行锁串行化检查。
