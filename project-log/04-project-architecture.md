...
配置导入由前端逐请求写入改为后端单事务入口，import_receipt 与业务数据一起提交以支持安全重试。资源引用兼容现存 JSON/URL，由 models/file_usage.go 按需解析；相关写入经 ResourceMutation 串行化，读取请求体发生在锁外。当前仅保证单进程的资源变更协调，多实例部署需要额外设计。
