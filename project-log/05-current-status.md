...
项目已从“fork 后清理”进入“独立开源项目发布后稳定性修复”阶段。当前重点是补齐本地联调与 Docker 管理实机验证，继续完善测试覆盖、安全边界、前端包体积和真实多语言翻译。
...
- 2026-05-24 已完成文件列表分页、SearchBox 保存失败反馈和文件删除部分成功反馈优化，避免 SearchBox 静默失败和文件图库全量加载。
- 2026-05-24 已完成导入导出、文件管理、搜索框、分组编辑、用户资料和外链安全边界优化：修正 GET header 透传、导出顺序、文件列表分页基础、部分删除反馈、草稿编辑和外链 `noopener`。
...
- 2026-06-04 已完成设置页和添加项目弹窗风格统一：使用 Apple-like 磨砂半透玻璃风格收口设置应用、添加项目弹窗、图标编辑器、按钮语义和表单 / 表格视觉；Docker 管理页补充 socket 挂载 / group 权限提示。
- 2026-06-04 已完成自定义壁纸首页观感优化：自定义壁纸下首页标题、分组、页脚、悬浮按钮和添加占位卡片切换为统一的亮色磨砂玻璃模式；上传新壁纸自动给出更合理的模糊、遮罩和图标文字默认值。
- 2026-06-05 已完成第四轮评审问题修复：补齐导航项目目标分组归属校验、批量新增错误处理、分组删除保留校验、自定义 CSS / JS 安全模式、请求钩子契约修正，并恢复 `project-log/` 本地知识库的 Git 忽略规则。默认管理员密码策略按用户确认保持现状。
...
- **代码质量改进第二阶段**：消灭前端 `any`、统一 Go 错误处理、继续收口剩余的边缘请求 / 状态问题。
...
| 当前环境无法代表用户服务器 Docker socket 权限 | 无法验证 Docker 管理实机操作 | 待处理 | 功能依赖 ZPanel 容器内可访问 Docker CLI / `/var/run/docker.sock`，不是只要求宿主机安装 Docker |
...
| 自定义 CSS / JS 缺少安全恢复入口 | 错误脚本或样式可能阻断进入设置页 | 已处理 | 2026-06-05 增加 `?safeMode=1` / `?zpanelSafeMode=1`，当前页面加载时跳过自定义 CSS / JS |
| favicon 后端抓取 SSRF 风险 | 登录用户可诱导服务端访问内网 / metadata 地址 | 已处理 | 2026-06-08 增加安全 HTTP 客户端，限制 scheme、DNS 解析目标和重定向目标 |
| 上传同源脚本面 | SVG / 任意文件经 `/uploads` 同源公开访问可能扩大 XSS / 钓鱼面 | 已处理 | 2026-06-08 上传白名单收紧为常见位图 / ico，移除 SVG 和任意文件上传 |

## 下一步

1. 做一次后端启动 + 前端联调，确认登录、验证码、首页、站点设置、导入导出、用户搜索、上传和 favicon 获取基础流程。
2. 在 Docker 环境中验证容器列表、操作和日志接口。
3. 增加更多后端单元测试，优先覆盖登录限流、导入导出格式校验、Docker 参数校验和上传边界。
4. 拆分前端入口和首页 chunk，逐步优化包体积。
5. 安排非中英语言真实翻译版。

## 任务交接

**当前任务**：第五轮安全与可用性优化已完成；下一步进入本地完整联调、Docker 实机验证和后续质量治理。

**已完成**：独立仓库重建、品牌清理、依赖升级、Docker Hub + GHCR 发布配置、PRO 功能表开源化第一版、README 多语言版本、产品 11 语言 locale 注册、中英文文案优化、CI、健康检查、1.0.3 版本推进、默认端口 6521、前端类型检查 / lint / build 验证、后端 CI 验证、第三轮 Bug 修复、安全渲染和类型收敛优化、导入导出 / 文件管理 / 搜索框 / 分组编辑 / 文件列表分页 / 外链安全边界优化。

**未完成**：本地完整联调、Docker 管理实机验证、更多 Go 后端测试覆盖、非中英语言真实翻译版、完整数据库/API 逆向。

**下一步建议**：先做一次完整本地联调，重点覆盖上传、favicon 获取、改密码后多端失效、用户搜索；随后补 Docker 管理实机验证。

**风险 / 阻塞**：当前环境缺少 Docker；后端仍缺测试覆盖；Docker socket 权限高；自定义 JS / CSS 已有安全模式但仍是高权限功能；前端大 chunk 仍需拆分。
