...
**修改内容**：
1. `project-log/11-code-review-log.md`、`project-log/10-planning-log.md`、`project-log/05-current-status.md` — 记录第五轮评审结论、采用方案、当前状态和后续验证重点。
2. `service/lib/safehttp/` — 新增安全 HTTP 客户端，限制 favicon 抓取的 URL scheme、localhost / 私网 / link-local / metadata 地址、DNS 解析目标和重定向目标。
3. `service/lib/siteFavicon/favico.go`、`service/lib/storage/storage.go`、`service/api/api_v1/panel/itemIcon.go` — favicon 页面抓取和图标下载接入安全 HTTP 客户端，并移除远程 SVG favicon 保存。
4. `service/api/api_v1/system/file.go`、`service/lib/storage/storage.go` — 上传增加 10MB 服务端限制，图片白名单移除 SVG，通用上传接口收紧为同一图片白名单。
5. `service/lib/cmn/base.go`、`service/api/api_v1/system/user.go`、`service/api/api_v1/panel/users.go`、`service/initialize/app.go`、`service/initialize/database/connect.go` — 密码哈希改为返回错误并增加 6-72 字节校验；改密码、管理员改密和删除用户后清空 token 缓存，避免缓存命中继续放行。
6. `service/lib/cmn/password_test.go`、`service/lib/safehttp/safehttp_test.go` — 增加密码长度 / 验证测试和安全 URL 校验测试。
7. `src/components/apps/Users/index.vue`、`src/typings/admin/adminUserManage.d.ts`、`src/locales/*.json` — 用户列表补搜索输入，统一请求字段为 `keyword`，同步 11 个 locale 的 `common.search`；补充公开访问用户未配置时的空值防御；修正角色标签的 Naive UI slot 写法。
8. `scripts/add-frontend-version.js`、`vite.config.ts`、`.gitignore` — 构建版本号改为写入 `.zpanel-build-version` 临时文件并由 Vite 注入，不再修改本地 `.env`。

**遇到的问题**：
- 首次重跑 Go 测试时路径使用了仓库根路径，但工作目录已在 `service/`，导致 `gofmt` 找不到文件。
- `storage` 去掉 `net/http` import 后仍使用 `http.StatusOK`，Go 编译失败。
- 管理员更新用户改为全局清空 token 缓存后，旧的 `userInfo` 变量不再使用，Go 编译失败。
- 浏览器复核账号管理入口时发现未配置公开访问用户会返回空 data，前端读取 `data.id` 产生控制台错误。
- 浏览器复核账号管理入口时发现角色标签存在 Naive UI `Non-function value encountered for default slot` warning。

**解决方式**：
- 用正确的 `service/` 内相对路径重跑 `gofmt` 和 `go test ./...`。
- 补回 `net/http` import。
- 删除不再需要的 `userInfo` 变量。
- 将公开访问用户读取改为 `data?.id || null`，并补充请求失败兜底。
- 将 `NTag` 渲染改为函数式 default slot。
...
- `cd service && go test ./...`
- `git diff --check`
- 本地启动 Go 后端和 Vite 开发服务器，使用浏览器验证账号管理入口、桌面 / 窄屏布局、搜索空结果与清空恢复。

**验证结果**：
- TypeScript 类型检查通过。
- ESLint 通过。
- Vite 生产构建通过；仍保留既有 `/custom/index.js` 非 module 提示、`/custom/index.css` 运行时解析提示和大 chunk 提示。
- Go 测试通过，新增 `lib/cmn` 和 `lib/safehttp` 测试通过。
- `git diff --check` 通过。
- 浏览器验证通过：账号管理入口无新增控制台错误；用户搜索不存在关键字显示 0，清空后恢复列表；桌面布局正常，390px 窄屏下搜索控件自动换行且无溢出；角色标签 slot warning 已修复。
- Docker 管理实机验证未运行，原因是本轮未启动 Docker 环境。
...
## 2026-06-05（v1.1.4 发布准备：第四轮评审修复发布）

**触发原因**：用户要求推进版本号、提交修改到 GitHub，并全渠道发布新版本。

**修改内容**：
1. `package.json` — 版本号从 `1.1.3` 推进到 `1.1.4`。
2. `service/assets/version` — 后端版本源从 `1|1.1.3` 推进到 `1|1.1.4`。
3. `CHANGELOG.md` — 新增 `1.1.4` 发布记录，覆盖导航项目 / 分组数据完整性修复、自定义 CSS / JS 安全模式、请求钩子修复、入口请求兜底和回归测试。
4. `README.md`、`README.zh-CN.md` — 固定版本镜像示例更新为 `vivalucas/zpanel:1.1.4`。
5. `project-log/05-current-status.md`、`project-log/08-env-config.md`、`project-log/06-dev-log.md` — 同步当前版本状态和发布准备记录。

**遇到的问题**：
- 本轮代码已包含 `project-log/` 从 Git 索引移除的变更；这是项目规范要求的本地知识库发布边界修正。

**解决方式**：
- 采用补丁版本 `1.1.4`，通过 tag `v1.1.4` 触发 GitHub Release、GHCR 和 Docker Hub 发布流程。
...
- `corepack pnpm build`
- `cd service && go test ./api/api_v1/panel ./...`

**验证结果**：
- 版本一致性通过：`1.1.4` / `1|1.1.4`。
- ESLint 全量通过。
- TypeScript 类型检查通过。
- Vite 生产构建通过；仍有既有的 `/custom/index.js` 非 module 提示、`/custom/index.css` 运行时解析提示和大 chunk 提示。
- Go 测试通过；新增 `panel` 包测试通过，其他后端包仍以 `[no test files]` 为主，`router` 包保留既有测试覆盖。

**本地产物清理**：
- 已删除 `corepack pnpm build` 生成的 `dist/`。
- `.env` 由构建脚本更新前端版本号，作为忽略文件保留。

---

## 2026-06-05（第四轮评审问题修复）

**触发原因**：用户要求在保留当前默认密码设计的前提下，复核第四轮评审发现的其他问题，并将确认属于 Bug 或可优化项的内容全部修复。

**修改内容**：
1. `service/api/api_v1/panel/itemIcon.go` — 增加导航项目目标分组归属校验，覆盖新增、编辑和批量新增；批量新增改为检查数据库写入错误后再返回成功。
2. `service/api/api_v1/panel/item_icon_group.go` — 分组删除的“至少保留一个”判断改为基于数据库实际命中的当前用户分组数量，避免无效 ID 绕过保护。
3. `src/utils/request/index.ts` — `afterRequest` 改为在请求成功、业务错误和网络错误后统一执行，修正请求钩子契约。
4. `src/App.vue` — 新增 `?safeMode=1` / `?zpanelSafeMode=1` 站点安全模式，当前页面加载时跳过自定义 CSS 和自定义 JavaScript，保留站点标题和图标。
5. `src/App.vue`、`src/views/login/index.vue` — 自查时补充公开登录配置请求失败兜底，避免后端未启动或短暂不可用时产生 unhandled rejection。
6. `README.md`、`README.zh-CN.md` — 补充自定义 CSS / JS 错误配置后的安全模式恢复方式。
7. `.gitignore` — 恢复忽略 `project-log/`；并通过 `git rm --cached -r project-log` 将已跟踪的项目知识库文件移出 Git 索引但保留本地文件。
8. `service/api/api_v1/panel/item_icon_security_test.go` — 新增后端回归测试，覆盖分组归属校验和“真实分组全删 + 无效 ID”绕过场景。
9. `project-log/11-code-review-log.md`、`project-log/06-dev-log.md`、`project-log/05-current-status.md` — 记录本轮评审结论、修复范围、验证结果和交接信息。

**遇到的问题**：
- 默认密码是确认保留的产品设计，不纳入本轮修改。
- `project-log/` 已被 Git 跟踪，仅修改 `.gitignore` 不能阻止继续发布，必须从 Git 索引移除；本地文件保留。
- 自定义 JS / CSS 属于产品显式能力，不能简单禁用，需要提供恢复路径。

**解决方式**：
- 后端接口统一在服务端验证目标分组属于当前用户，不依赖前端下拉选项保证合法。
- 删除分组时先统计当前用户真实命中的待删分组数量，再判断删除后是否仍保留至少一个分组。
- 通过 URL 安全模式临时跳过自定义 CSS / JS，用户可登录后进入设置页清理错误配置。
...
- `corepack pnpm build`
- `cd service && go test ./api/api_v1/panel ./...`
- 本地启动 Vite 开发服务器，使用浏览器验证普通登录页和 `?safeMode=1#/login` 安全模式登录页。

**验证结果**：
- `corepack pnpm lint` 通过。
- `corepack pnpm type-check` 通过。
- `corepack pnpm build` 通过；仍有既有的 `/custom/index.js` 非 module 提示、`/custom/index.css` 运行时解析提示、大 chunk 提示和 `vite:svg-icons` 插件耗时提示。
- `cd service && go test ./api/api_v1/panel ./...` 通过；新增 `panel` 包测试通过，其他后端包仍以 `[no test files]` 为主，`router` 包保留既有测试覆盖。
- 浏览器验证普通登录页和安全模式登录页均能正常渲染登录表单，桌面和移动宽度无横向溢出；后端未启动时仅保留预期的代理错误和网络错误提示，不再出现 unhandled rejection。
...
## 2026-09-12（全项目评审修复与用户流程优化）

**触发原因**：用户在只读评审后明确授权“落实文档和代码修复优化”。先记录 10/11，再修改代码。

**修改内容**：
1. 登录/中间件/路由 — 验证码尺寸和独立限流、可信代理配置、资源写入互斥与已删除相对 URL 拒绝。读取请求体在互斥锁外进行，重复资源路径在每次请求中去重查询。
2. 用户 API — 至少保留一个启用管理员（事务及 MySQL 管理员行锁），密码变更与 session 撤销原子化，登出撤销失败明确返回错误。
3. models/file_usage.go、文件 API、图库 — 根据现存业务数据识别图片用途，阻止有引用文件删除，替换本人引用并隐藏他人私有标题；壁纸保存成功后应用，失败保留草稿。
4. import.go、import_receipt、导入导出组件 — 服务端事务、追加/覆盖、持久幂等、预览和重名提示；导出读取失败即中止，排序 0 不再被替换。明确配置 JSON 不携带上传图片。
5. Docker API/界面 — 请求上下文取消、30 秒超时、2 MiB 输出上限、行级操作状态和具体容器停止/重启确认。未操作实际 Docker 容器。
6. favicon — 基于最终响应 URL 解析相对路径并保留查询参数，HTML 2 MiB、候选 20 个上限。
7. 导航/搜索 — 前后端过滤危险协议，搜索引擎编辑 Enter 不触发搜索，保存失败提示在选择面板关闭后仍可见并可重试。
8. 回归测试/CI — 新增 Go 测试与 scripts/review-tests.mjs，新增 pnpm test:review 并接入 CI；新文案同步所有 locale key，维持既有非中英基线策略。
9. 文档 — 更新 00/01/02/03/04/05/07/08/10/11/12；当前版本保持 1.1.7，记录知识库已跟踪事实，不改 Git 规则。

**遇到的问题**：初始 Go 缓存不完整，默认代理超时；浏览器发现旧导入提示与覆盖模式冲突；自检发现锁内读取请求体会被慢客户端拖住其他写入。

**解决方式**：当前命令使用 goproxy.cn 下载锁定依赖，未关闭校验或改全局配置；修正所有 locale 的导入说明；先限量读取解析请求体，再获取资源锁。

**验证方式与结果**：
- `cd service && go test ./...`：通过。
- `go test -race ./api/api_v1/panel ./api/api_v1/system ./api/api_v1/middleware ./models ./router ./lib/navigation ./lib/siteFavicon`：通过；中间件后续变更后单独再跑 race 通过。
- Go 回归覆盖：验证码尺寸、限流隔离、可信代理、唯一管理员回滚、session 撤销故障下密码回滚、导入失败回滚/重复重试/内容冲突/配置合并、旧 URL 引用识别和跨账号替换隔离、已删除资源拒绝、危险 URL、favicon 相对路径/查询参数、Docker 取消和输出上限。
- `pnpm run test:review`：通过，覆盖导航 URL、导入 schema/checksum、导出失败与排序。
- `pnpm run lint`：通过，无 warning/error。
- `pnpm run build`（含 type-check）：通过。保留既有 custom 静态资源和大 chunk 提示；入口约 856 kB，首页约 472 kB，未在本轮扩展为包体优化。
- 隔离数据库和上传目录真实 API 冒烟：上传、引用查询、阻止删除、替换后删除、旧引用拒绝、导入幂等、唯一管理员和危险 URL 拒绝全部通过。
- 浏览器联调：登录、导入预览/重名提示/追加导入后首页展示、导出弹窗、图库用途、最终提示及窄视口菜单通过；文件详情做了可视布局检查。
- `git diff --check`：通过；Go 格式检查通过。

**验证限制**：真实 Docker 容器操作、MySQL/Redis 实机及多实例并发未验证；当前资源互斥为单进程方案。Go M1 CPU 依赖有既有 C 编译器 warning，不影响通过。

**本地产物清理**：已关闭临时浏览器页面、停止测试服务，删除隔离数据库/图片/配置/二进制/测试文件/构建日志及 dist/.zpanel-build-version。截图仅用于工具内查看，未保留文件。清理命令最初因禁止 rm -f 类形式被审核拒绝，随后使用校验根路径且拒绝符号链接的定点清理成功完成。node_modules 和正常依赖缓存保留用于后续开发。无提交、推送或发布。

## 2026-09-12（部署文档完善与 1.1.8 发布准备）

**触发原因**：用户要求落实文档优化、推进版本、提交 GitHub main、打 tag 并发布全部现有渠道；暂不做实机。

**修改内容**：中英文 README 与新增 docs/deployment 双语运维参考；同步 project-log 07/08/10/11；版本源推进 1.1.8，CHANGELOG 汇总前轮已合入修复与部署边界。

**验证方式**：检查配置路径和入口行为、文档链接/代码块语法、版本一致性；本地前端检查；提交后跟进 CI、Release、双注册表双架构。

**验证结果**：33 个文档 Bash 代码块语法、相对链接目标、1.1.8 双版本源一致性检查通过；pnpm test:review、lint、build（含类型检查）通过，仅保留既有 custom 静态资源与大包警告。发布工作流同步打包新增运维文档，并从 CHANGELOG 提取本版本说明。远端结果追加于发布完成记录。

**本地产物清理**：已清理本轮 dist/ 和 .zpanel-build-version，保留既有依赖目录；未进行实机部署。
