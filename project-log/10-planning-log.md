...
**替代关系**：补充 ADR-003 / ADR-004 中的 PRO 功能开源化安全边界。

**背景与需求**：第五轮全项目评审发现，ZPanel 已有的 favicon 抓取、文件上传、会话缓存、密码更新、用户搜索和构建版本写入存在真实风险或可用性问题。用户确认按最佳实践优化，同时要求不移除自定义 CSS / JS、公共图库等既有产品能力。

**采用的方案**：

- Favicon 获取继续保留，但后端请求前后都校验 URL：只允许 `http` / `https`，禁止 localhost、私网段、link-local、metadata 地址和跳转后的不安全目标。
- 上传能力继续保留，但图片上传移除 SVG 直传；通用上传接口收紧为同一图片白名单，并增加服务端大小限制。
- `/uploads` 仍作为公开资源服务，但通过限制可上传类型降低同源脚本执行面。
- 修改密码、管理员改密、降权、禁用用户时，除撤销 DB session 外同步清理该用户缓存 token，避免缓存命中继续放行。
- 密码后端增加长度校验，bcrypt 返回错误时向调用方返回业务错误，不再 panic。
- 用户列表补搜索输入，并统一字段名为 `keyword`。
- 构建版本号不再写入本地 `.env`，改为生成被忽略的临时版本文件供 Vite 读取。

**备选方案**：

1. 完全禁用 favicon 后端抓取
   - 优点：SSRF 面最小。
   - 缺点：会破坏导航面板自动获取站点图标的核心体验，也不利于 NAS / Homelab 内网站点场景。
   - 放弃原因：保留功能并加目标地址校验更符合产品定位。

2. 对 `/uploads` 全部加鉴权
   - 优点：资源访问更私密。
   - 缺点：图标、壁纸、公共图库和静态资源直链会受影响，公开模式体验下降。
   - 放弃原因：当前产品定义需要浏览器直接访问上传资源，先通过类型限制降低风险。

3. 移除自定义 CSS / JS 和 footer HTML
   - 优点：XSS 风险最低。
   - 缺点：违背 ZPanel 已确认的高自定义能力。
   - 放弃原因：该能力按管理员可信功能保留，并已有 `?safeMode=1` 恢复入口。

4. 保持 `add-version` 写 `.env`
   - 优点：实现简单。
   - 缺点：构建修改开发者本地配置文件，CI / 本地验证副作用不清晰。
   - 放弃原因：版本产物应是可再生成文件，不应污染 `.env`。

**决策依据**：

- 自托管项目要兼顾内网可用性和公网部署安全，安全策略应优先限制危险目标和危险文件类型，而不是粗暴移除核心能力。
- 会话撤销必须尽量立即生效，尤其是改密、降权和禁用账号。
- 构建命令应可重复、低副作用，避免改写开发者本地配置。

**改动范围**：

- `service/lib/siteFavicon/favico.go`
- `service/lib/storage/storage.go`
- `service/api/api_v1/system/file.go`
- `service/lib/cmn/base.go`
- `service/api/api_v1/system/user.go`
- `service/api/api_v1/panel/users.go`
- `src/components/apps/Users/index.vue`
- `src/typings/admin/adminUserManage.d.ts`
...
### ADR-20260912 [2026-09-12] 全项目评审修复与交互优化

**状态**：已采用（用户明确授权“落实文档和代码修复优化”）。

**背景与需求**：上一轮只读评审确认验证码资源边界、文件引用、管理员保留、导入导出、代理限流、Docker 超时与 favicon URL 解析问题；用户现已授权落地。

**采用的方案**：验证码固定安全边界并限流；资源删除基于当前业务数据核查真实引用并展示用途（兼容现存 URL 数据）；管理员角色和密码/session 变更使用事务；配置导入使用服务端事务、预览和幂等标识，导出严格失败；默认不信任代理，仅允许显式可信地址；Docker 请求支持超时、取消、有限输出与行级确认；favicon 遵循 URL 解析规则并限制抓取资源。导航 URL 限制危险协议；保存失败保留草稿并提供重试。

**备选方案及否定原因**：不采用仅前端回滚导入（网络失败下不可靠）；不恢复任意代理信任（可伪造 IP）；不只依赖空引用表（已有 URL 数据未迁移）；不把配置 JSON 宣称为完整资源备份。

**改动范围**：相关 Go API、模型辅助函数、路由与配置，Vue 导入导出／图库／Docker／搜索和跳转，多语言 key，针对性测试，以及 project-log 01/02/03/05/06/07/08/11/12。

**验证计划**：安装锁定依赖，Go 测试、前端类型检查/lint/build；增加事务回滚、角色保留、引用保护、URL/验证码边界回归测试。不得对真实 Docker 容器执行停止等操作；实机限制如实记录。不改变现有 Git 跟踪规则、不提交或发布。

## ADR-20260912-RELEASE：部署文档完善与 1.1.8 发布

**状态**：已采用（用户明确要求落实文档、推进版本、提交 main、打 tag、全渠道发布）。

**方案**：修订中英文部署指引，补齐配置、生效方式、持久化、备份恢复和代理边界；将 package.json 与 service/assets/version 同步到 1.1.8，更新 CHANGELOG。先检查并提交 main，CI 通过后创建 v1.1.8 标签，跟进 GitHub Release、Docker Hub、GHCR 及双架构镜像。

**范围与取舍**：不做用户暂缓的实机部署；不新增业务功能或自动备份功能。复用现有发布工作流，不把 main 推送当作镜像发布。文档验收与远端发布核验不能替代 Ubuntu 实机恢复演练。
