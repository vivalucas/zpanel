...
| 4 | Codex | 2026-06-05 | 3 确认 Bug + 3 改进项 + 1 流程修正 | — |
| 5 | Codex | 2026-06-08 | 6 确认问题 + 2 待确认边界 + 7 优化项 | — |

---

## 当前状态

第三轮评审已在云端 `project-log/` 基线基础上完成，详细逐项复核见 `2026-05-23-review.md`。后续复查修复与再评审见 `2026-05-25-review.md`。2026-06-05 第四轮复核确认并修复了导航项目 / 分组数据完整性、自定义 CSS / JS 恢复路径、请求钩子契约和 `project-log/` Git 忽略规则冲突。第一轮、第二轮和第三轮的全部发现见下方。
...
第四轮已确认并纳入本轮修复：
- 分组删除接口按请求 ID 数量判断“至少保留一个”，无效 ID 可绕过保护并删除全部真实分组。
- 导航项目创建 / 编辑 / 批量新增缺少目标分组归属校验，可能把项目写入不存在或他人分组，造成数据不可见或跨用户数据完整性问题。
- 批量新增导航项目忽略数据库写入错误，失败时仍返回成功。
- 自定义 CSS / JS 是既有高权限功能，但缺少安全恢复入口，错误脚本或样式可能阻断进入设置页。
- 请求工具 `beforeRequest` / `afterRequest` 契约不完整，成功请求不会触发 `afterRequest`。
- `project-log/` 文档要求本地知识库不随 GitHub 发布，但 `.gitignore` 中对应规则被注释且文件已被跟踪。

第四轮复核后不改：
- 默认管理员账号和 `12345678` 默认密码保持现有设计；仅继续在 README 中提示首次登录后修改密码。

第五轮已确认并纳入本轮修复：
- `GetSiteFavicon` 后端抓取缺少 localhost、私网段、link-local、metadata 地址和重定向目标限制，存在 SSRF 风险。
- `/uploads` 同源公开服务上传文件，且图片上传允许 SVG、通用上传不限扩展名，扩大 XSS / 钓鱼面。
- 改密码、降权或禁用用户后，`UserToken` 缓存命中会短暂绕过 session 撤销状态。
- 密码缺少后端长度校验，bcrypt 失败会 panic。
- 用户列表搜索字段前后端不一致，且缺少搜索输入。
- 账号管理在未配置公开访问用户时读取空 `data.id`，打开入口会产生前端控制台错误。
- `pnpm run build` 会写入本地 `.env`，构建存在不必要副作用。

第五轮复核后不改：
- 自定义 footer / CSS / JS 属于管理员可信能力，保留功能；继续依赖安全模式作为恢复入口。
- 公共图库按当前产品定义展示所有 public 上传资源，暂不改为按用户隔离。

## 最近自检

| 日期 | 范围 | 结果 | 未覆盖 |
|------|------|------|--------|
| 2026-06-08 | 第五轮安全与可用性复核 | `pnpm run type-check`、`pnpm run lint`、`pnpm run build`、`cd service && go test ./...` 均通过；发现 SSRF、上传同源脚本面、会话失效窗口、密码校验、用户搜索和构建副作用问题 | Docker 管理实机操作、浏览器完整联调 |
...
**评审日期**：2026-06-13
**评审范围**：全项目底层机制、登录控制及多端一致性
**评审方法**：沙盘推演与边界场景测试，着重复核此前五轮改动引入的次生风险和性能盲区。

## 一、第六轮结论

本轮共排查出 4 个疑似风险点，经过严密的二次推演复核，最终确认为：
...
1. **多端登录缓存未同步更新**：`UpdateInfo` 仅删除当前 Token 缓存。复核确认 `global.UserToken` 的 TTL 仅为 1 分钟。在 HomeLab 场景下，1 分钟的“最终一致性”是轻微且可接受的。
2. **修改密码引发全站会话缓存失效**：`UpdatePassword` 中调用了 `global.UserToken.Flush()` 误杀所有用户缓存。复核发现这是前序回合为了“强制阻断被降权/改密后的旧 Token 苟活 1 分钟”而特意增加的安全补丁。在小用户量场景下，全局 Flush 是极其合理且务实的安全架构决策。
3. **前端登录网络异常无 UI 反馈**：`login/index.vue` 的 `catch` 块留空。复核确认 `src/utils/request/index.ts` 已经全局接管了网络错误并主动提示 `message.error`，此处留空是为了阻断错误不再抛出。

### 确认为真实 Bug

**BUG-21：登录限流的 O(N) 遍历造成 DoS 风险**
- **位置**：`service/api/api_v1/middleware/login_rate_limit.go:29-33`
- **严重性**：高（若暴露公网）
- **描述**：原有的 `LoginRateLimit` 拦截器在每次请求时都在互斥锁 `ipRecordsMu.Lock()` 保护下全量遍历 `ipRecords` map 以清理过期记录。因为 Go 的 map 没有自动收缩机制，若遭遇大规模分布式爆破试探，字典会迅速膨胀，导致每个正常的登录请求都被迫进行长耗时的 O(N) 锁等待。
- **影响**：可能导致整个系统的登录服务被阻塞瘫痪。
...
| 编号 | 文件 | 改动 |
|------|------|------|
| BUG-21 | `service/api/api_v1/middleware/login_rate_limit.go` | 移除 `LoginRateLimit` 函数中的主干遍历清理逻辑。新增 `init()` 函数并启动一个后台 Goroutine，每 5 分钟定期在锁保护下清理一次过期的 IP 记录，使主链路耗时稳定在 O(1)。 |

### 未修复项（后续处理建议）

| 项目 | 说明 |
|------|------|
| 登录限流灵活度 | 当前硬编码每分钟 10 次纯 IP 限制，NAT 网络下易误伤。建议未来升级为“IP+账号”维度或采用令牌桶算法。 |

## 2026-09-12 全项目评审（实施前记录）

已先阅读 README、状态、背景、架构、功能设计和历史决策。当前代码 1.1.7，部分知识库仍为 1.1.6；知识库已被 Git 跟踪，与旧本地忽略要求冲突，本轮保留现状并注明历史。

### 已确认

1. 验证码尺寸无上限、无独立限流；已核对 base64Captcha v1.3.5 直接分配图片。
2. 文件引用表仅建表与查询，无业务写入；图标/壁纸 URL 引用无法阻止误删。
3. 编辑角色没有最后管理员保护；前端可降权自己，初始化和密码重置无法恢复。
4. 导出遇到业务错误仍返回空分组/列表，可能提示成功。
5. 导入跨多个请求无事务或幂等，失败后重试重复。
6. 默认不信任代理是正确安全基线，但缺少可信代理配置，反代用户共用 IP 限额。
7. Docker 命令没有取消/超时；参数白名单和管理员权限已存在，排除注入误报。
8. favicon 相对路径错误拼到根目录且查询参数被删除。

### 待确认

- 导航/搜索 URL 未限制危险协议，需复核调用边界。
- 密码更新与 session 撤销非原子且忽略错误，需回归覆盖。

### 用户体验建议

导入预览与追加/覆盖说明；配置导出与完整备份区分；图库引用展示与替换；Docker 行级状态和停机确认；保存失败草稿/重试；搜索编辑 Enter 隔离及模板校验。

### 初次验证限制

只读轮未修改文件；离线 Go 测试依赖缺失，safehttp 通过，未做前端和 Docker 实机测试。本轮用户已授权按 ADR-20260912 实施，完成后补充验证结果。

### 2026-09-12 实施与复核结果

上述 8 项已确认问题均已落实。两项待确认风险在实现调用链及回归测试中进一步确认：危险 URL 可进入原跳转路径，现增加前后端协议过滤；session 撤销错误此前不能阻止密码提交，现通过 SQLite 故障触发器验证事务回滚。

体验建议已落实为导入预览/追加覆盖、配置备份边界、图库用途和本人引用替换、Docker 行级状态/确认、保存失败草稿重试及搜索 Enter 隔离。浏览器发现旧导入文案矛盾已纠正；窄视口菜单需点标题展开，已实测可用，未将 AX 树未变化误判为功能故障。
