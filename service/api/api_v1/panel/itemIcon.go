...
func (a *ItemIcon) Edit(c *gin.Context) {
	...
	if req.ID != 0 {
		// 修改
		updateField := []string{"IconJson", "Title", "Url", "LanUrl", "Description", "OpenMethod", "UserId", "ItemIconGroupId"}
		if req.Sort != 0 {
			updateField = append(updateField, "Sort")
		}
		var count int64
		if err := global.Db.Model(&models.ItemIcon{}).Where("id=? AND user_id=?", req.ID, userInfo.ID).Count(&count).Error; err != nil {
