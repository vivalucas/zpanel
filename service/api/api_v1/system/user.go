...
func (a *UserApi) GetInfo(c *gin.Context) {
	userInfo, exists := base.GetCurrentUserInfo(c)
	if !exists {
...
func (a *UserApi) UpdatePasssword(c *gin.Context) {
	...
	if v, err := mUser.GetUserInfoByUid(userInfo.ID); err != nil {
		...
		if !cmn.VerifyPassword(params.OldPassword, v.Password) {
			// 旧密码不正确
...
func (a *UserApi) GetReferralCode(c *gin.Context) {
	currentUserInfo, exists := base.GetCurrentUserInfo(c)
	if !exists {
		apiReturn.ErrorByCode(c, 1001)
		return
	}
