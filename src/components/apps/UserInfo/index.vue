...
import { NButton, NCard, NDivider, NForm, NFormItem, NInput, NSelect, NTag, useDialog, useMessage } from 'naive-ui'
...
const switchAccounts = ref<Array<{ token: string; userInfo: User.Info; updatedAt: number }>>([])
const switchAccountStorageKey = 'ZPANEL_ACCOUNTS'
...
function removeSwitchAccount(userId?: number) {
  switchAccounts.value = switchAccounts.value.filter(item => item.userInfo.id !== userId)
  saveSwitchAccounts()
}
...
<template>
  <div class="bg-slate-200 dark:bg-zinc-900 p-2 h-full">
    ...
    <NCard style="border-radius:10px" class="mt-[10px]" size="small">
      ...
      <div v-if="switchAccounts.length === 0" class="text-slate-500 text-sm">
        {{ $t('apps.userInfo.noSavedAccounts') }}
      </div>
