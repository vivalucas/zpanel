...
interface ExportJsonResult {
  ...
  addStyleData(data: Panel.panelConfig): ExportJsonResult
  exportFile(): void
  string(): string
}
...
export function exportJson(appVersion?: string): ExportJsonResult {
  ...
  return {
    ...
    addStyleData(data: Panel.panelConfig) {
      jsonData.styleConfig = data
      return this
    },
    ...
    exportFile() {
      ...
      if (jsonString) {
        ...
        link.download = `ZPanel-Data${dayjs().format('YYYYMMDDHHmm')}.zpanel.json`
        link.click()
      }
    },
...
export interface ImportJsonResult {
  ...
  getStyleConfig: () => Panel.panelConfig | null
}
...
export function importJsonString(jsonString: string): ImportJsonResult | null {
  ...
  if ((data.version as number) < ALLOW_LOW_VERSION)
  ...
  return {
    ...
    getStyleConfig: (): Panel.panelConfig | null => {
      return jsonStruct.styleConfig || null
    },
  }
}

function transformJson(jsonData: Record<string, unknown>): JsonStructure | null {
  // 检查必须存在的键
...
function generateMD5(jsonString: string): string {
  try {
    // 解析 JSON 字符串
    const data: Record<string, unknown> = JSON.parse(jsonString) as Record<string, unknown>
    // 移除 md5 字段及其对应的值
    removeMD5Field(data)
    // 将修改后的 JSON 对象转换回字符串
...
function removeMD5Field(obj: Record<string, unknown>): void {
